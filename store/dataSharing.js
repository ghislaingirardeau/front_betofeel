...
export function shareRequest() {
  const checkDemand = this.$parent.userOndemand.findIndex(
    (element) => element.pseudo === this.searchPseudo.toLowerCase()
  );
  const checkShared = this.$parent.userShared.findIndex(
    (element) => element.pseudo === this.searchPseudo.toLowerCase()
  );
  if (checkShared != -1) {
    ...
    fetch("https://web-production-b214.up.railway.app/api/share/searchUser", {
      method: "POST",
      headers: {
      ...
      if (response.ok) {
        response.json().then((data) => {
          if (data.message === "undefined") {
            ...
            const newSharing = {
              connectTo: data.idFind,
              pseudo: this.searchPseudo,
              status: "on demand",
            };
...
export function responseDemand(res, idFrom, pseudo) {
  ...
  fetch(
    "https://web-production-b214.up.railway.app/api/share/responseSharing",
    {
      method: "PUT",
      headers: {
        "content-type": "application/json",
    ...
    if (response.ok) {
      response.json().then((data) => {
        if (res === "authorized") {
          /* si authorized alors je mets à jour le data des composants sans reload */
...
export function removeSharing(user) {
  ...
  axios
    .put("https://web-production-b214.up.railway.app/api/share/remove", data, {
      headers: {
        "content-type": "application/json",
        Authorization: "Bearer " + token,
      },
    ...
    .then((response) => {
      ...
      const updateOneShare = this.userShared.findIndex(
        (element) => element.connectTo === userTo
      );
