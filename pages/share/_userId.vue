<template>
  <div class="container align-items-center share_tree">
    <headerBar :pseudo="pseudo">
      ...
      <div class="col-6">
        <b-button to="/" @click.native="disconnect" variant="outline-primary"
      ...
      <section class="row justify-content-center mb-3">
        <article class="col-11">
          <transition name="fade">
            <h2 v-if="dataLoad" class="col-12 mb-0 mb-lg-3 mt-2 text-center">
              Voici l'avatar de {{ pseudoShare }}
            </h2>
          </transition>
...
export default {
  ...
  mounted() {
    ...
    fetch("https://web-production-b214.up.railway.app/api/share/feelingUser", {
      method: "POST",
      headers: {
      ...
      if (response.ok) {
        ...
        response.json().then((data) => {
          console.log(
            data.message
          ); /* renvoie error du backend sur le frontend */
        });
      }
    });
  },
